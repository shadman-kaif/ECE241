...
module lab5Part2Quartus (input [2:0] SW, input CLOCK_50, output [9:0] LEDR, output [6:0] HEX0);

				assign LEDR[9:0] = 10'b0000000000; // all LEDs remain off
				wire [4:0] disp_Count;
						
				speedController S1 (.f_Select(SW[1:0]), .clock(CLOCK_50), .reset(SW[2]), .countState(disp_Count));
				BCD_to_HEX_Decoder D1 (.C(disp_Count[3:0]), .HEX(HEX0));
						
endmodule // lab5Part2

/*  */
module lab5part2 (input [1:0] f_Select, input clock, reset, 
				...
				fourBit_Counter C1 (.clock(clock), .reset(reset), .enable(dwnClk_Enable), .Q(countState));
